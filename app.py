...
# Streamlit 급등 신호 대시보드 (모바일 지원 포함)
...
def detect_signals(df):
    ...
    for i in range(2, len(df)):
        win = df.iloc[i-2:i+1]
        rsi_cross = any((win.iloc[j]['RSI_9'] > win.iloc[j]['RSI']) and (win.iloc[j-1]['RSI_9'] <= win.iloc[j-1]['RSI']) for j in range(1, 3))
        ...
        for j in range(1, 3):
            ...
            if (prev_close <= prev_ma5 or prev_close <= prev_ma20) and (curr_close > curr_ma5 and curr_close > curr_ma20):
                ma_cross = True
        if rsi_cross and obv_ok and ma_cross:
