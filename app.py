...
def compute_signals(df):
    ...
    for i in range(2, len(df)):
        ...
        if rsi_ok and obv_ok and ma_ok:
            ...
            is_premium = pd.notna(rsi_val) and 50 <= rsi_val <= 65
            signals.append({
                '날짜': df.index[i].strftime('%Y-%m-%d'),
                '종가': round(df.iloc[i]['Close'], 2),
                'RSI(14)': round(rsi_val, 2) if pd.notna(rsi_val) else None,
                'OBV': round(obv_val, 0) if pd.notna(obv_val) else None,
                'OBV_20': round(obv20_val, 0) if pd.notna(obv20_val) else None,
                '프리미엄': is_premium
            })
