...
def compute_signals(df):
    ...
    for i in range(2, len(df)):
        ...
        for j in range(1, 3):
            try:
                rsi_9_now = win.at[win.index[j], 'RSI_9']
                rsi_14_now = win.at[win.index[j], 'RSI_14']
                rsi_9_prev = win.at[win.index[j-1], 'RSI_9']
                rsi_14_prev = win.at[win.index[j-1], 'RSI_14']
                if pd.notna(rsi_9_now) and pd.notna(rsi_14_now) and pd.notna(rsi_9_prev) and pd.notna(rsi_14_prev):
